...
class invoice_facturae_html(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context=None):
        ...
        self.localcontext.update({
            ...
            'set_referencia': self._set_dict_nombre,
            'set_tipo': self._set_dict_tipo,
            'set_symbol': self._set_currency_symbol,
            'set_global_invoice_line': self._set_invoice_line,
    ...
    def _set_dict_notes(self, o):
        ...
        return invoice_br.comment
        
    def _set_dict_nombre(self, o):
        id_source = int(o.id_source)
        invoice_obj = self.pool.get('account.invoice')
        invoice_br = invoice_obj.browse(self.cr, self.uid, id_source, context=None)
        return invoice_br.name

    def _set_dict_tipo(self,o):
        id_source = int(o.id_source)
        invoice_obj= self.pool.get('account.invoice')
        invoice_br = invoice_obj.browse(self.cr, self.uid, id_source, context=None)
        tipo=''
    ...
    def _set_invoice_line(self, o):
        ...
        invoice_br = invoice_obj.browse(self.cr, self.uid, id_source, context=None)
        invoice_lines = []
        for linea in invoice_br.invoice_line:
            pedimentos = []
            if linea.pedimentos_manuales_ids:
                for lpd in linea.pedimentos_manuales_ids:
                    xpd={
                        'aduana': lpd.aduana,
                        'fecha': lpd.fecha,
                        'numero': str(lpd.numero), 
                        }
                    pedimentos.append(xpd)
            xline = {
                    'cantidad': str(linea.quantity),
                    'descripcion': str(linea.name),
                    'unidad': str(linea.uos_id.name),
                    'precio_unitario': str(linea.price_unit),
                    'importe': str(linea.price_subtotal),
